...
Опциональные переменные окружения (значения по умолчанию в `docker-compose.yml`):
...
- `OCR_MAX_FILE_MB` — максимальный размер исходного файла в МБ для загрузок через UI и для ингеста из watch (по умолчанию `50`, в текущем стенде выставлено `200`). Файл читается в память целиком — значение должно соответствовать доступной RAM хоста. Для watch-документов превышение всё равно фиксируется в БД как `status='error'` и файл сразу перемещается в `inbox/errors/` вместе с сайдкаром — на OCR он не отправляется.
